...
public abstract class CommonResponse {
    ...
    public static Response send(Response.Status status, String message) {
        Integer code = status.getFamily() == Response.Status.Family.SUCCESSFUL ? CodeMessage.CODE_SUCCESS : CodeMessage.CODE_defaDEFAULT_FAILURE;
    ...
    public static Response op(Runnable executor, Consumer<Exception> ex){
        try {
            executor.run();
